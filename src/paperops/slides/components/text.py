...
from paperops.slides.layout.autofit import (
    TextStyle,
    build_intrinsic_size,
    measure_text_intrinsic,
)
    ...
    def measure(self, constraints: Constraints, theme) -> IntrinsicSize:
        ...
        if not content:
            height = (
                ...
                else (pt / 72.0) * self.line_spacing + self.margin_y
            )
            width = (
                self.width if self.width is not None else (constraints.max_width or 0.0)
            )
        ...
        intrinsic = build_intrinsic_size(
            measure_text_intrinsic(content, style, max_width_inches=wrap_width)
        )
        if self.width is not None:
            intrinsic = IntrinsicSize(
                ...
                intrinsic.preferred_height,
            )
        if self.height is not None:
            intrinsic = IntrinsicSize(
                intrinsic.min_width, intrinsic.preferred_width, self.height, self.height
            )
        if self.min_width is not None:
            intrinsic = IntrinsicSize(
                max(intrinsic.min_width, self.min_width),
                intrinsic.preferred_width,
                intrinsic.min_height,
                intrinsic.preferred_height,
            )
        if self.min_height is not None:
            intrinsic = IntrinsicSize(
                intrinsic.min_width,
                intrinsic.preferred_width,
                max(intrinsic.min_height, self.min_height),
                intrinsic.preferred_height,
            )
        return intrinsic.clamp(constraints)

    def preferred_size(self, theme, available_width: float) -> tuple[float, float]:
        intrinsic = self.measure(
            Constraints(max_width=max(available_width, 0.0)), theme
        )
...
class BulletList(LayoutNode):
    ...
    def measure(self, constraints: Constraints, theme) -> IntrinsicSize:
        ...
        for item in self.items:
            ...
            usable_width = max(
                (wrap_width if wrap_width is not None else base_width)
                - indent_width
                - self.margin_x,
                0.4,
            )
            ...
            item_intrinsic = measure_text_intrinsic(
                text, style, max_width_inches=usable_width
            )
        ...
        width = (
            self.width
            if self.width is not None
            else (wrap_width if wrap_width is not None else min_width)
        )
        intrinsic = IntrinsicSize(
            ...
            min_height=(
                ...
                if self.height is not None
                    ...
                    (pt / 72.0) * self.line_spacing + self.margin_y, preferred_height
                )
            ),
            preferred_height=(
                self.height if self.height is not None else preferred_height
            ),
        )
        return intrinsic.clamp(constraints)
