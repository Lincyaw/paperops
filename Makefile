...
tag:
	...
	VERSION="$$VERSION" uv run python -c "from pathlib import Path; import os, re; p=Path('pyproject.toml'); s=p.read_text(); p.write_text(re.sub(r'^version = \".*\"', 'version = \"' + os.environ['VERSION'] + '\"', s, count=1), encoding='utf-8')"
